...
inline double MatrixMul_32x32_Step1_MultAndOffset_2_d(__local double* aLocal, __local double* bLocal)
{
...
inline double MatrixMul_32x32_Step1_MultAndOffset_4_d(__local double* aLocal, __local double* bLocal)
{
    //sums four items, and offset the array for this to happen again
    //Using this method in a pyramidal way will give better summation accuracy
    return MatrixMul_32x32_Step1_MultAndOffset_2_d(aLocal,bLocal) + MatrixMul_32x32_Step1_MultAndOffset_2_d(aLocal+2,bLocal+64);
}

inline double MatrixMul_32x32_Step1_MultAndOffset_8_d(__local double* aLocal, __local double* bLocal)
{
    //sums 8 items, and offset the array for this to happen again
    //Using this method in a pyramidal way will give better summation accuracy
    return MatrixMul_32x32_Step1_MultAndOffset_4_d(aLocal,bLocal) + MatrixMul_32x32_Step1_MultAndOffset_4_d(aLocal+4,bLocal+128);
}
inline double MatrixMul_32x32_Step1_MultAndOffset_16_d(__local double* aLocal, __local double* bLocal)
{
    //sums 16 items, and offset the array for this to happen again
    //Using this method in a pyramidal way will give better summation accuracy
    return MatrixMul_32x32_Step1_MultAndOffset_8_d(aLocal,bLocal) + MatrixMul_32x32_Step1_MultAndOffset_8_d(aLocal+8,bLocal+256);
}
inline double MatrixMul_32x32_Step1_MultAndOffset_32_d(__local double* aLocal, __local double* bLocal)
{
    //sums 16 items, and offset the array for this to happen again
    //Using this method in a pyramidal way will give better summation accuracy
    return MatrixMul_32x32_Step1_MultAndOffset_16_d(aLocal,bLocal) + MatrixMul_32x32_Step1_MultAndOffset_16_d(aLocal+16,bLocal+512);
}
kernel void MatrixMul_32x32_Step1_Multiplication_d(__global double* A, __global double* B, int nRowsA_patch, int nColsARowsB_patch, int nColsB_patch, __local double* aLocal, __local double* bLocal, __global double* C)
{  
   ...
   double sum = MatrixMul_32x32_Step1_MultAndOffset_32_d(aLocal,bLocal);;//MatrixMul_32x32_Step1_MultAndOffset_32(aLocal,bLocal);
    ...
    */
    ...
    C[offsetC + localRowA * 32 + localColB] =sum;//A[MatrixMul_32x32_Step1_GetIndex(patch_col_A_row_B,patch_row_A, nColsARowsB_patch) + threadId];
return;
}
...
kernel void MatrixMul_32x32_Step2_Reduction_d(__global double* C, int nRowsA_patch, int nColsARowsB_patch, int nColsB_patch)
{  
...
kernel void MatrixMul_32x32_Step2_Reduction_ReduceOneItem_d(__global double* C, int nRowsA_patch, int nColsARowsB_patch, int nColsB_patch, int stepSize)
{  
...
//but 1,2,3,4,5 -->  1+5,2,3,4 for this method (now we are an even number of items long and can call the other method 
   //Performs the second step of matrix multiplication using a 32x32 patch based approach - it takes two patches that need to be added and adds them
   //this step will need be repeated for as many reductions are required (64 cols in A = 1, 128 cols in A = 2, etc)
   //Input:
   //   C:                  result from matrixMul_32x32_Step1 (see that algorithm for details on data ordering) 
